...
function Header({
  ...
  const [currentPage, setCurrentPage] = useState(2); // Set default to 2
  const [selectedPlace, setSelectedPlace] = useState(null); // New state to track selected place
  ...
  const handleClick = (place) => {
    setCurrentPage(2);
    setSelectedPlace(place); // Menetapkan tempat yang dipilih
    setIsOpen(true); // Membuka popup
  };
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://api-sumatra.vercel.app/places",
        );
          ...
          top5Places.map(async (place) => {
            try {
              const imageResponse = await axios.get(
                `https://api-sumatra.vercel.app/img/${place.image}`,
                {
                  responseType: "blob",
                },
              );
              ...
              console.error(
                `Error fetching image for place ${place.name}:`,
                error,
              );
  ...
  return (
    <div className="relative min-h-[75vh]">
      ...
      />
      <div className="absolute bottom-0 left-0 text-left z-10 w-full p-4">
        <h1
          ref={discoverRef}
        ...
        <div
          ref={textRef}
          className="p-2 backdrop-blur-md mb-16 rounded-full inline-flex border border-4 border-white items-center relative scale-100 opacity-100 px-5 pe-7 cursor-pointer"
          onClick={() => handleClick(places[currentPlaceIndex])} // Pass the current place
        >
